...
var MessagesJaJP = map[string]string{
	...
	"config.ignored_error_status_codes":      "エラー無視ステータスコード",
	"config.ignored_error_status_codes_desc": "これらの上流 HTTP ステータスコードは正常な応答として扱い、クライアントへそのまま転送し成功として記録します。リトライ・キーの失敗カウント・ブラックリスト化は行いません。フェイルオーバーステータスコードより優先されます。カンマ区切りと範囲指定に対応（例：405）。グループごとに個別上書き可能。MCP のようにクライアントが能動的にプローブし、上流が 405 等で正常に拒否するプロトコルで有用です。",
