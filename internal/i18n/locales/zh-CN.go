...
var MessagesZhCN = map[string]string{
	...
	"config.ignored_error_status_codes":      "忽略错误状态码",
	"config.ignored_error_status_codes_desc": "上游返回这些 HTTP 状态码时视为正常响应，直接透传给客户端并记为成功，不重试、不计密钥失败、不拉黑。优先级高于故障转移状态码。支持逗号分隔和范围，例如：405。分组可单独覆盖此值。常用于 MCP 等协议客户端主动探测、上游用 405 等正常拒绝的场景。",
