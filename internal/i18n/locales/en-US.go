...
var MessagesEnUS = map[string]string{
	...
	"config.ignored_error_status_codes":      "Ignored Error Status Codes",
	"config.ignored_error_status_codes_desc": "Upstream HTTP status codes treated as normal responses: passed through to the client and logged as success, with no retry, no key failure count, and no blacklisting. Priority is higher than failover status codes. Supports comma-separated values and ranges, e.g.: 405. Groups can override this value individually. Useful for protocols like MCP where the client actively probes and the upstream rejects with codes such as 405.",
